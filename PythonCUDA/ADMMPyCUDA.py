...
@cuda
def copy_array_complex(m, n, x, y):

    startX, startY = cuda.grid(2)
    gridX = cuda.gridDim.x * cuda.blockDim.x;
    gridY = cuda.gridDim.y * cuda.blockDim.y;

    for i in (StartX, y.size()[0], cuda.gridDim.x)
    {
        for j in (startY, y.size()[1], cuda.gridDim.y)
            y[i*n+j][x] = x[i*n+j][x];
            y[i*n+j][x] = x[i*n+j][y];
        }
...
#parallelized function for multiplying
# two dense, large matrices.
...
@cuda
def matrix_scale(int m, int n, float2 *x, float k)
{
    // x = kx
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    int col = blockIdx.y * blockDim.y + threadIdx.y;
...
@cuda
def circshift(int m, int n, float2 *x, float2 *y)
{
...
@cuda
def circshift_reverse_and_real(int m, int n, float2 *x, float2 *y)
{
    // Put reverse circshifted x into y
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    int col = blockIdx.y * blockDim.y + threadIdx.y;
...
void Xi_update(int m, int n, float2 *draft1, float2 *X, float2 *xi)
{
    ...
    row = blockIdxx * blockDim.x + threadIdx.x;
...
void Eta_update(int m, int n, float2 *psi_V, float2 *U, float2 *eta)
{
    ...
    #evaluates to blockDim * blockIdx + threadIdx
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    int col = blockIdx.y * blockDim.y + threadIdx.y;
    for (int i = row; i < m; i += (gridDim.x * blockDim.x))
    {
...
def main()
    int numTests = 200;
    record = run_admm_image(numTests); #double *
    # // for (int i = 0; i < numTests; i++)
    # // {
    # //     printf("Test %d\n", i + 1);
    # //     record[i] = run_ista_image(i+1);
    # // }
    filname = "../test/CUDA_Result/ADMM_0_200_single.csv";
